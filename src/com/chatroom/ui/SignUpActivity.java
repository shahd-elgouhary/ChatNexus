...
public class SignUpActivity {
	...
	public SignUpActivity(ClientModel cm) throws IOException {
		clientModel = cm;
		jFrame = new JFrame("ChatNexus - Sign Up");
		...
		jLabelsignUptitle.setFont(new Font("Segoe UI", Font.BOLD, 24));
		jLabelsignUptitle.setForeground(Config.getTextPrimary());

		jLabelsignin = new JLabel("<html><a href=''>Already a user? Sign In here</a></html>");
		jLabelsignin.setFont(new Font("Segoe UI", Font.PLAIN, 12));
		jLabelsignin.setForeground(Config.getPrimary());
	...
	private void ListeningEvents() {
		jBtnSignUp.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				String username = jTvUsername.getText().trim();
				...
				try {
					...
					if (response.getId() == Response.Type.SIGN_UP.ordinal()) {
						if (response.getSuccess()) {
							...
							SwingUtilities.invokeLater(() ->
								JOptionPane.showMessageDialog(null, response.getContents(), "Signup Failed", JOptionPane.ERROR_MESSAGE)
							);
						}
					}
